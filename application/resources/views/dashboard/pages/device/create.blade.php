    ...
    <div class="row">
        <div class="col-4">
            <div class="mb-5 mr-2 card">
                <div class="card-body">
                    <h4 class="card-title">ایجاد دستگاه</h4>
                    <form class="forms-sample" wire:submit.prevent="create" _lpchecked="1">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" wire:model.defer="device.category_id" name="category" id="category">
                                    <option>دسته را انتخاب کنید</option>
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" wire:key="option_category_{{$category->id}}">{{ $category->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="number" min="0" wire:model.defer="device.price" class="form-control" id="price"
                                       placeholder="نام">
                                @error('device.price')
                                <p class="mt-2 text-small text-danger">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="file"  wire:model.defer="image" class="form-control" id="image" placeholder="نام">
                                <img class="w-100" src="{{ $image?->temporaryUrl() }}" alt="">
                                @error('image')
                                <p class="mt-2 text-small text-danger">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        <button type="submit" class="mr-2 btn btn-primary">ایجاد</button>
                    </form>
                </div>
            </div>
        </div>
                ...
                <div class="card-body">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9" x-data="{}">
                            <textarea name="body" id="body" class="w-full description" wire:model.defer="device.body"></textarea>
                            @error('device.body')
                            <p class="mt-2 text-small text-danger">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9" x-data="{}">
                            <textarea name="description" id="description" class="w-full form-control description" wire:model.defer="device.description"></textarea>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            Livewire.hook('message.sent', (message, component) => {
                if (message.component.fingerprint.name === 'dashboard.pages.device.create'){
                    destroyEditor()
                }
            })
        });
